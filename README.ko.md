...
MorphoSNN은 절지동물의 분산 신체 지능에서 출발하여, 형태 결합형 SNN 및 뉴로모픽 제어 구조를 탐색하는 공개 연구 코드베이스입니다.

- 문제: 현대 AI 시스템은 여전히 중앙집중형 고밀도 연산에 크게 의존하지만, 생물학적 시스템은 신체 분절, 다리, 반사 루프, 국소 제어를 통해 제어를 수행합니다.
- 가설: 절지동물은 지능이 중앙 뇌에만 있는 것이 아니라, 마디, 다리, 국소 반사, CPG, 형태 구조 전체에 분산될 수 있음을 보여줍니다.
- 현재 저장소: 이 저장소는 CPG 및 보행 제어 중심의 초기 실험, 공개 연구 노트, 시각 개념 자료를 포함하며, 이는 더 넓은 뉴로모픽 컴퓨팅 가설의 첫 번째 task-family입니다.

MorphoSNN은 초기 연구 저장소이며, 생물학적 충실성, production readiness, 일반적 SNN 우위, 측정된 에너지 절감 효과, AlexNet 수준의 성과를 주장하지 않습니다.
